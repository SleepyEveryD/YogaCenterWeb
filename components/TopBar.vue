    ...
    <nav>
      <div class="topbar">
        ...
        <ul v-if="!mobile" class="navigation">
          <li v-for="link in links" :key="link.path" class="nav-item" @mouseenter="activeDropdown = link.title" @mouseleave="activeDropdown = null">
            ...
            <ul v-if="link.dropdown && activeDropdown === link.title" class="dropdown-menu">
              <li v-for="sublink in link.dropdown" :key="sublink.path">
                <NuxtLink :to="sublink.path">{{ sublink.title }}</NuxtLink>
              </li>
            </ul>
          </li>
...
.topbar {
  ...
  border-bottom: 5px solid #48A6A7;
  color: #ffffff;
}
